...
export const kilometerFormatter: AxisLabelsFormatterCallbackFunction =
    function (this: AxisLabelsFormatterContextObject): string {
        return (typeof this.value === 'number' ? this.value : 0) / 1000 + 'km'
    }

export const meterDataLabelFormatter: DataLabelsFormatterCallbackFunction =
    function (this: Point): string {
        if (!this.index) {
...
export const kilometerDataLabelFormatter: DataLabelsFormatterCallbackFunction =
    function (this: Point): string {
        if (!this.index) {
            if (this.index === 0 && this.series.data[0].y) {
...
export const defaultDataLabelFormatter: DataLabelsFormatterCallbackFunction =
    function (this: Point): string {
        if (
            this.index !== undefined &&
            this.series.data[this.index].y !== undefined
        ) {
            return (
                Math.round(this.series.data[this.index].y || 0)?.toString() ||
                ''
            )
        }
        return ''
...
export function getSerieFormatterByType(
    serieType?: string
